 ...
 */
static int ec_privkey_import_der(const secp256k1_context* ctx, uint8_t* out32, const uint8_t* privkey,
                                 size_t privkeylen) {
  const uint8_t* end = privkey + privkeylen;
  memset(out32, 0, 32);
  /* sequence header */
  if (end - privkey < 1 || *privkey != 0x30u) { return 0; }
  privkey++;
  /* sequence length constructor */
  if (end - privkey < 1 || !(*privkey & 0x80u)) { return 0; }
  ptrdiff_t lenb = *privkey & ~0x80u;
  privkey++;
  if (lenb < 1 || lenb > 2) { return 0; }
  if (end - privkey < lenb) { return 0; }
  /* sequence length */
  ptrdiff_t len = privkey[lenb - 1] | (lenb > 1 ? privkey[lenb - 2] << 8 : 0u);
  privkey += lenb;
  if (end - privkey < len) { return 0; }
  /* sequence element 0: version number (=1) */
  if (end - privkey < 3 || privkey[0] != 0x02u || privkey[1] != 0x01u || privkey[2] != 0x01u) { return 0; }
  privkey += 3;
  /* sequence element 1: octet string, up to 32 bytes */
  if (end - privkey < 2 || privkey[0] != 0x04u) { return 0; }
  ptrdiff_t oslen = privkey[1];
  privkey += 2;
  if (oslen > 32 || end - privkey < oslen) { return 0; }
 ...
 */
static int ec_privkey_export_der(const secp256k1_context* ctx, uint8_t* privkey, size_t* privkeylen,
                                 const uint8_t* key32, int compressed) {
...
CPrivKey CKey::GetPrivKey() const {
  ...
  int ret = ec_privkey_export_der(secp256k1_context_sign, (uint8_t*)&privkey[0], &privkeylen, begin(),
...
bool CKey::Sign(const uint256& hash, std::vector<uint8_t>& vchSig, uint32_t test_case) const {
  ...
  assert(ret);
  secp256k1_ecdsa_signature_serialize_der(secp256k1_context_sign, (uint8_t*)vchSig.data(), &nSigLen, &sig);
...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
  ...
  uint256 hash;
  CHash256().Write((uint8_t*)str.data(), str.size()).Write(rnd, sizeof(rnd)).Finalize(hash.begin());
...
bool CKey::Derive(CKey& keyChild, ChainCode& ccChild, unsigned int nChild, const ChainCode& cc) const {
  ...
  bool ret = secp256k1_ec_privkey_tweak_add(secp256k1_context_sign, (uint8_t*)keyChild.begin(), vout.data());
