...
bool static IsLowDERSignature(const valtype& vchSig, ScriptError* serror) {
  if (!IsValidSignatureEncoding(vchSig)) { return set_error(serror, SCRIPT_ERR_SIG_DER); }
  ...
  if (!CPubKey::CheckLowS(vchSigCopy)) return set_error(serror, SCRIPT_ERR_SIG_HIGH_S);
...
bool EvalScript(vector<vector<uint8_t> >& stack, const CScript& script, unsigned int flags,
  ...
  if (script.size() > 10000) return set_error(serror, SCRIPT_ERR_SCRIPT_SIZE);
  int nOpCount = 0;
  ...
  } catch (...) { return set_error(serror, SCRIPT_ERR_UNKNOWN_ERROR); }
...
class CTransactionSignatureSerializer {
 ...
 public:
  ...
  template <typename S> void SerializeScriptCode(S& s) const {
    ...
    if (itBegin != scriptCode.end()) s.write((char*)&itBegin[0], it - itBegin);
  }
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, unsigned int flags,
  ...
  if ((flags & SCRIPT_VERIFY_P2SH) && scriptPubKey.IsPayToScriptHash()) {
    ...
    if (!EvalScript(stackCopy, pubKey2, flags, checker, serror))
      // serror is set
      return false;
    if (stackCopy.empty()) return set_error(serror, SCRIPT_ERR_EVAL_FALSE);
