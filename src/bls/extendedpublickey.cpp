...
uint32_t ExtendedPublicKey::GetVersion() const { return version; }

uint8_t ExtendedPublicKey::GetDepth() const { return depth; }

uint32_t ExtendedPublicKey::GetParentFingerprint() const { return parentFingerprint; }

uint32_t ExtendedPublicKey::GetChildNumber() const { return childNumber; }
...
bool operator==(ExtendedPublicKey const &a, ExtendedPublicKey const &b) {
  BLS::AssertInitialized();
  return (a.GetPublicKey() == b.GetPublicKey() && a.GetChainCode() == b.GetChainCode());
}

bool operator!=(ExtendedPublicKey const &a, ExtendedPublicKey const &b) { return !(a == b); }
