...
void CalculateParams(ZerocoinParams &params, CBigNum N, string aux, uint32_t securityLevel) {
  ...
  if (NLen < 1023) { throw std::runtime_error("Modulus must be at least 1023 bits"); }

  // Verify that "securityLevel" is  at least 80 bits (minimum).
  if (securityLevel < 80) { throw std::runtime_error("Security level must be at least 80 bits."); }
  ...
  params.coinCommitmentGroup =
      deriveIntegerGroupParams(calculateSeed(N, aux, securityLevel, STRING_COMMIT_GROUP), pLen, qLen);
  ...
  params.accumulatorParams.accumulatorPoKCommitmentGroup =
      deriveIntegerGroupParams(calculateSeed(N, aux, securityLevel, STRING_AIC_GROUP), qLen + 300, qLen + 1);
  ...
  uint32_t resultCtr;
  params.accumulatorParams.accumulatorQRNCommitmentGroup.g =
      generateIntegerFromSeed(NLen - 1, calculateSeed(N, aux, securityLevel, STRING_QRNCOMMIT_GROUPG), &resultCtr)
          .pow_mod(CBigNum(2), N);
  params.accumulatorParams.accumulatorQRNCommitmentGroup.h =
      generateIntegerFromSeed(NLen - 1, calculateSeed(N, aux, securityLevel, STRING_QRNCOMMIT_GROUPH), &resultCtr)
          .pow_mod(CBigNum(2), N);
  ...
  for (uint32_t count = 0; count < MAX_ACCUMGEN_ATTEMPTS && params.accumulatorParams.accumulatorBase.isOne(); count++) {
    params.accumulatorParams.accumulatorBase =
        constant.pow_mod(CBigNum(2), params.accumulatorParams.accumulatorModulus);
  }
...
void calculateGroupParamLengths(uint32_t maxPLen, uint32_t securityLevel, uint32_t *pLen, uint32_t *qLen) {
  *pLen = *qLen = 0;
  ...
  if (*pLen > maxPLen) { throw std::runtime_error("Modulus size is too small for this security level."); }
}
...
IntegerGroupParams deriveIntegerGroupParams(uint256 seed, uint32_t pLen, uint32_t qLen) {
  ...
  calculateGroupModulusAndOrder(seed, pLen, qLen, &(result.modulus), &(result.groupOrder), &pSeed, &qSeed);

  std::cout << "Modulus = " << result.modulus.ToString() << " " << result.modulus.bitSize() << "\n";
  std::cout << "Group Order = " << result.groupOrder.ToString() << " " << result.groupOrder.bitSize() << "\n";
  ...
  std::cout << "g = " << result.g.ToString() << " " << result.g.bitSize() << "\n";
  std::cout << "h = " << result.h.ToString() << " " << result.h.bitSize() << "\n";
...
SerialNumberGroupParams deriveIntegerGroupFromOrder(CBigNum &groupOrder) {
  SerialNumberGroupParams result;
  ...
  *resultGroupOrder = generateRandomPrime(qLen, seed, &qseed, &qgen_counter);
  ...
  for (; pgen_counter <= ((4 * pLen) + old_counter); pgen_counter++) {
    ...
    *resultModulus = (CBigNum(2) * t * (*resultGroupOrder) * p0) + CBigNum(1);
    ...
    if ((resultModulus->gcd(z - CBigNum(1))).isOne() && (z.pow_mod(p0, (*resultModulus))).isOne()) {
...
CBigNum calculateGroupGenerator(uint256 seed, uint256 pSeed, uint256 qSeed, CBigNum modulus, CBigNum groupOrder,
  ...
  if (index > 255) { throw std::runtime_error("Invalid index for group generation"); }
...
CBigNum generateRandomPrime(uint32_t primeBitLen, uint256 in_seed, uint256 *out_seed, uint32_t *prime_gen_counter) {
  ...
  if (primeBitLen < 2) { throw std::runtime_error("Prime length is too short"); }
