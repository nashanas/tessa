...
UniValue ping(const UniValue& params, bool fHelp) {
  ...
  for (CNode* pNode : vNodes) { pNode->fPingQueued = true; }
...
UniValue getpeerinfo(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() != 0)
    throw runtime_error(
        ...
        "    \"subver\": \"/Tessa Core:x.x.x.x/\",  (string) The string version\n"
  ...
  for (const CNodeStats& stats : vstats) {
    ...
    if (!(stats.addrLocal.empty())) obj.push_back(Pair("addrlocal", stats.addrLocal));
    ...
    if (fStateStats) {
      ...
      for (int height : statestats.vHeightInFlight) { heights.push_back(height); }
...
UniValue getaddednodeinfo(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() < 1 || params.size() > 2)
    throw runtime_error(
        ...
        "\nArguments:\n"
        "1. dns        (boolean, required) If false, only a list of added nodes will be provided, otherwise connected "
        ...
        "    \"addresses\" : [\n"
        "       {\n"
        "         \"address\" : \"192.168.0.201:44444\",  (string) The club server host and port\n"
...
UniValue getnetworkinfo(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() != 0)
    throw runtime_error(
        ...
        "  \"subversion\": \"/Tessa Core:x.x.x.x/\",     (string) the server subversion string\n"
        ...
        "  ,...\n"
        "  ],\n"
        "  \"relayfee\": x.xxxxxxxx,                (numeric) minimum relay fee for non-free transactions in club/kb\n"
...
UniValue setban(const UniValue& params, bool fHelp) {
  ...
  if (fHelp || params.size() < 2 || (strCommand != "add" && strCommand != "remove"))
    throw runtime_error(
        ...
        "\nArguments:\n"
        "1. \"ip(/netmask)\" (string, required) The IP/Subnet (see getpeerinfo for nodes ip) with a optional netmask "
        "(default is /32 = single ip)\n"
        "2. \"command\"      (string, required) 'add' to add a IP/Subnet to the list, 'remove' to remove a IP/Subnet "
        "from the list\n"
        "3. \"bantime\"      (numeric, optional) time in seconds how long (or until when if [absolute] is set) the ip "
        "is banned (0 or empty means using the default time of 24h which can also be overwritten by the -bantime "
        "startup argument)\n"
        "4. \"absolute\"     (boolean, optional) If set, the bantime must be a absolute timestamp in seconds since "
  ...
  if (strCommand == "add") {
    ...
    bool absolute = false;
    if (params.size() == 4) absolute = params[3].get_bool();

    isSubnet ? CNode::Ban(subNet, BanReasonManuallyAdded, banTime, absolute)
             : CNode::Ban(netAddr, BanReasonManuallyAdded, banTime, absolute);

    // disconnect possible nodes
    while (CNode* bannedNode = (isSubnet ? FindNode(subNet) : FindNode(netAddr))) bannedNode->CloseSocketDisconnect();
  } else if (strCommand == "remove") {
