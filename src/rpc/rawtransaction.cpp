...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, UniValue& out, bool fIncludeHex) {
  ...
  if (fIncludeHex) out.push_back(Pair("hex", HexStr(scriptPubKey.begin(), scriptPubKey.end())));
...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry) {
  ...
  entry.push_back(Pair("version", tx.nTransactionVersion));
...
UniValue listunspent(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() > 4)
    throw runtime_error(
        ...
        "3. \"addresses\"    (string) A json array of club addresses to filter\n"
        "    [\n"
        "      \"address\"   (string) club address\n"
        "      ,...\n"
        "    ]\n"
        "4. watchonlyconfig  (numberic, optional, default=1) 1 = list regular unspent transactions, 2 = list only "
        "watchonly transactions,  3 = list all unspent transactions (including watchonly)\n"
        ...
        "    \"address\" : \"address\",  (string) the club address\n"
  ...
  RPCTypeCheck(params, {UniValue::VNUM, UniValue::VNUM, UniValue::VARR, UniValue::VNUM});
  ...
  if (params.size() > 2) {
    ...
    for (unsigned int inx = 0; inx < inputs.size(); inx++) {
      ...
      if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Club address: ") + input.get_str());
  ...
  if (params.size() > 3) {
    ...
    if (nWatchonlyConfig > 3 || nWatchonlyConfig < 1) nWatchonlyConfig = 1;
  }
...
UniValue createrawtransaction(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() != 2)
    throw runtime_error(
        ...
        "    {\n"
        "      \"address\": x.xxx   (numeric, required) The key is the club address, the value is the btc amount\n"
        "      ,...\n"
        "    }\n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() != 1)
    throw runtime_error(
        ...
        "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
        "         \"addresses\" : [           (json array of string)\n"
        "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) club address\n"
        "           ,...\n"
        "         ]\n"
        "       }\n"
        "     }\n"
...
UniValue signrawtransaction(const UniValue& params, bool fHelp) {
  ...
  while (!ssData.empty()) {
    ...
    } catch (const std::exception&) { throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TX decode failed"); }
  }

  if (txVariants.empty()) throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Missing transaction");
  ...
  if (params.size() > 1 && !params[1].isNull()) {
    ...
    for (unsigned int idx = 0; idx < prevTxs.size(); idx++) {
      ...
      if (fGivenKeys && scriptPubKey.IsPayToScriptHash()) {
        RPCTypeCheckObj(prevOut, {{"txid", UniValue::VSTR},
                                  {"vout", UniValue::VNUM},
  ...
  if (!vErrors.empty()) { result.push_back(Pair("errors", vErrors)); }
...
UniValue getspentzerocoinamount(const UniValue& params, bool fHelp) {
  ...
  if (!input.scriptSig.IsZerocoinSpend()) return -1;
