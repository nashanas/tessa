...
UniValue blockheaderToJSON(const CBlockIndex* blockindex) {
  ...
  if (pnext) result.push_back(Pair("nextblockhash", pnext->GetBlockHash().GetHex()));
  return result;
}
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false) {
  ...
  for (auto denom : libzerocoin::zerocoinDenomList) {
    zkpObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom * COIN))));
  }
  zkpObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
  result.push_back(Pair("Zkpsupply", zkpObj));
  ...
  if (fVerbose) {
    ...
    for (const auto& entry : mempool.mapTx) {
      ...
      for (const CTxIn& txin : tx.vin) {
        if (mempool.exists(txin.prevout.hash)) setDepends.insert(txin.prevout.hash.ToString());
      }

      UniValue depends(UniValue::VARR);
      for (const string& dep : setDepends) { depends.push_back(dep); }
...
UniValue getrawmempool(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() > 1)
    throw runtime_error(
        ...
        "    \"fee\" : n,              (numeric) transaction fee in club\n"
...
UniValue getblock(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() < 1 || params.size() > 2)
    throw runtime_error(
        ...
        "  \"Zkpsupply\" :\n"
        "  {\n"
        "     \"1\" : n,            (numeric) supply of 1 ZKP denomination\n"
        "     \"5\" : n,            (numeric) supply of 5 ZKP denomination\n"
        "     \"10\" : n,           (numeric) supply of 10 ZKP denomination\n"
        "     \"50\" : n,           (numeric) supply of 50 ZKP denomination\n"
        "     \"100\" : n,          (numeric) supply of 100 ZKP denomination\n"
        "     \"500\" : n,          (numeric) supply of 500 ZKP denomination\n"
        "     \"1000\" : n,         (numeric) supply of 1000 ZKP denomination\n"
        "     \"5000\" : n,         (numeric) supply of 5000 ZKP denomination\n"
        "     \"total\" : n,        (numeric) The total supply of all ZKP denominations\n"
        "  }\n"
        "}\n"
...
UniValue gettxout(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() < 2 || params.size() > 3)
    throw runtime_error(
        ...
        "     \"addresses\" : [          (array of string) array of club addresses\n"
        "     \"clubaddress\"   	 	(string) club address\n"
        "        ,...\n"
        "     ]\n"
        "  },\n"
...
UniValue getchaintips(const UniValue& params, bool fHelp) {
  ...
  for (const CBlockIndex* block : setTips) {
    ...
    if (chainActive.Contains(block)) {
      ...
      // This block is fully validated, but no longer part of the active chain. It was probably the active block once,
      // but was reorganized.
      status = "valid-fork";
...
UniValue getfeeinfo(const UniValue& params, bool fHelp) {
  ...
  if (nBlocks < 0 || nStartHeight <= 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "invalid start height");
  ...
  for (int i = nStartHeight; i <= nBestHeight; i++) {
    ...
    CBlock block;
    if (!ReadBlockFromDisk(block, pindex)) throw JSONRPCError(RPC_DATABASE_ERROR, "failed to read block from disk");
...
UniValue invalidateblock(const UniValue& params, bool fHelp) {
  ...
  if (!state.IsValid()) { throw JSONRPCError(RPC_DATABASE_ERROR, state.GetRejectReason()); }
