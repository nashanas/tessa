...
UniValue GetNetworkHashPS(int lookup, int height) {
  ...
  if (lookup <= 0) lookup = pb->nHeight % 2016 + 1;
...
UniValue getnetworkhashps(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() > 2)
    throw runtime_error(
        ...
        "\nArguments:\n"
        "1. blocks     (numeric, optional, default=120) The number of blocks, or -1 for blocks since last difficulty "
        "change.\n"
...
UniValue getgenerate(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() != 0)
    throw runtime_error(
        ...
        "It is set with the command line argument -gen (or club.conf setting gen)\n"
...
UniValue setgenerate(const UniValue& params, bool fHelp) {
  ...
  if (pwalletMain == nullptr) throw JSONRPCError(RPC_METHOD_NOT_FOUND, "Method not found (disabled)");
  ...
  {
    // previous was back setting 'args' here
...
UniValue gethashespersec(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() != 0)
    throw runtime_error(
        ...
        "\nResult:\n"
        "n            (numeric) The recent hashes per second when generation is on (will return 0 if generation is "
        "off)\n"

        "\nExamples:\n" +
        HelpExampleCli("gethashespersec", "") + HelpExampleRpc("gethashespersec", ""));

  if (GetTimeMillis() - nHPSTimerStart > 8000) return (int64_t)0;
...
UniValue getblocktemplate(const UniValue& params, bool fHelp) {
  if (fHelp || params.size() > 1)
    throw runtime_error(
        ...
        "(in upiv); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not "
        "including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't "
        "one\n"
        ...
        "      ,...\n"
        "  ],\n"
        "  \"coinbaseaux\" : {                  (json object) data that should be included in the coinbase's scriptSig "
        "content\n"
        "      \"flags\" : \"flags\"            (string) \n"
        "  },\n"
        "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including "
        "the generation award and transaction fees (in upiv)\n"
  ...
  if (vNodes.empty()) throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Club is not connected!");

  if (IsInitialBlockDownload()) throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Club is downloading blocks...");
