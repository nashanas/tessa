...
AccumulatorMap::AccumulatorMap(libzerocoin::ZerocoinParams* params) {
  ...
  for (auto& denom : zerocoinDenomList) {
    ...
    mapAccumValues.insert(make_pair(denom, std::move(uptr)));
  }
}

// Reset each accumulator to its default state
void AccumulatorMap::Reset() { Reset(params); }

void AccumulatorMap::Reset(libzerocoin::ZerocoinParams* params2) {
  this->params = params2;
  mapAccumValues.clear();
  for (auto& denom : zerocoinDenomList) {
...
bool AccumulatorMap::Load(uint256 nCheckpoint) {
  for (auto& denom : zerocoinDenomList) {
    ...
    mapAccumValues.at(denom)->setValue(bnValue);
  }
  return true;
}
...
void AccumulatorMap::Load(const AccumulatorCheckpoints::Checkpoint& checkpoint) {
  for (auto it : checkpoint) mapAccumValues.at(it.first)->setValue(it.second);
}
...
bool AccumulatorMap::Accumulate(const PublicCoin& pubCoin, bool fSkipValidation) {
  ...
  if (denom == CoinDenomination::ZQ_ERROR) return false;

  if (fSkipValidation)
    mapAccumValues.at(denom)->increment(pubCoin.getValue());
  else
    mapAccumValues.at(denom)->accumulate(pubCoin);
  return true;
}

// Get the value of a specific accumulator
CBigNum AccumulatorMap::GetValue(CoinDenomination denom) {
  if (denom == CoinDenomination::ZQ_ERROR) return CBigNum(0);
  return mapAccumValues.at(denom)->getValue();
}
...
uint256 AccumulatorMap::GetCheckpoint() {
  ...
  for (auto& denom : zerocoinDenomList) {
    CBigNum bnValue = mapAccumValues.at(denom)->getValue();
