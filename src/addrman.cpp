...
int CAddrInfo::GetTriedBucket(const uint256& nKey) const {
  uint64_t hash1 = (CHashWriter() << nKey << GetKey()).GetHash().GetLow64();
  uint64_t hash2 =
      (CHashWriter() << nKey << GetGroup() << (hash1 % ADDRMAN_TRIED_BUCKETS_PER_GROUP)).GetHash().GetLow64();
...
int CAddrInfo::GetBucketPosition(const uint256& nKey, bool fNew, int nBucket) const {
  uint64_t hash1 = (CHashWriter() << nKey << (fNew ? 'N' : 'K') << nBucket << GetKey()).GetHash().GetLow64();
...
void CAddrMan::ClearNew(int nUBucket, int nUBucketPos) {
  ...
  if (vvNew[nUBucket][nUBucketPos] != -1) {
    ...
    if (infoDelete.nRefCount == 0) { Delete(nIdDelete); }
  }
}
...
void CAddrMan::Good_(const CService& addr, int64_t nTime) {
  ...
  LogPrint(TessaLog::ADDRMAN, "Moving %s to tried\n", addr.ToString());
...
bool CAddrMan::Add_(const CAddress& addr, const CNetAddr& source, int64_t nTimePenalty) {
  ...
  if (pinfo) {
    ...
    if (nFactor > 1 && (GetRandInt(nFactor) != 0)) return false;
  } else {
  ...
  if (vvNew[nUBucket][nUBucketPos] != nId) {
    ...
    if (fInsert) {
      ...
      if (pinfo->nRefCount == 0) { Delete(nId); }
    }
  }
  return fNew;
}
...
CAddress CAddrMan::Select_() {
  ...
  if (nTried > 0 && (nNew == 0 || GetRandInt(2) == 0)) {
    ...
    while (1) {
      ...
      CAddrInfo& info = mapInfo[nId];
      if (GetRandInt(1 << 30) < fChanceFactor * info.GetChance() * (1 << 30)) return info;
      fChanceFactor *= 1.2;
    }
  } else {
    // use a new node
