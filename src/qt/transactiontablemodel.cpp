    ...
    if (idx >= 0 && idx < cachedWallet.size()) {
      ...
      if (lockMain) {
        ...
        if (lockWallet && rec->statusUpdateNeeded()) {
          ...
          if (mi != wallet->mapWallet.end()) { rec->updateStatus(mi->second); }
        }
      }
      return rec;
    }
  ...
  QString describe(TransactionRecord* rec, int unit) {
    {
      LOCK2(cs_main, wallet->cs_wallet);
      std::map<uint256, CWalletTx>::iterator mi = wallet->mapWallet.find(rec->hash);
      if (mi != wallet->mapWallet.end()) { return TransactionDesc::toHTML(wallet, mi->second, rec, unit); }
    }
    return QString();
  }
};
...
QString TransactionTableModel::formatTxDate(const TransactionRecord* wtx) const {
  if (wtx->time) { return GUIUtil::dateTimeStr(wtx->time); }
  return QString();
}
...
QString TransactionTableModel::lookupAddress(const std::string& address, bool tooltip) const {
  ...
  if (label.isEmpty() || tooltip) { description += QString(" (") + QString::fromStdString(address) + QString(")"); }
  return description;
}
...
QVariant TransactionTableModel::addressColor(const TransactionRecord* wtx) const {
  switch (wtx->type) {
    ...
    case TransactionRecord::Generated:
    default:
      // To avoid overriding above conditional formats a default text color for this QTableView is not defined in
      // stylesheet, so we must always return a color here
      return COLOR_BLACK;
  }
}
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord* wtx, bool showUnconfirmed,
  ...
  QString str = BitcoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit, false,
                                     separators);
  if (showUnconfirmed) {
    if (!wtx->status.countsForBalance) { str = QString("[") + str + QString("]"); }
  }
  return QString(str);
}
...
QVariant TransactionTableModel::data(const QModelIndex& index, int role) const {
  ...
  switch (role) {
    ...
    case Qt::ForegroundRole:
      ...
      if (index.column() == Amount && (rec->credit + rec->debit) < 0) { return COLOR_NEGATIVE; }
      if (index.column() == ToAddress) { return addressColor(rec); }
...
QModelIndex TransactionTableModel::index(int row, int column, const QModelIndex& parent) const {
  ...
  if (data) { return createIndex(row, column, priv->index(row)); }
  return QModelIndex();
}
...
static void NotifyTransactionChanged(TransactionTableModel* ttm, CWallet* wallet, const uint256& hash,
                                     ChangeType status) {
  // Find transaction in wallet
  std::map<uint256, CWalletTx>::iterator mi = wallet->mapWallet.find(hash);
