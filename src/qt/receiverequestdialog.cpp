...
void QRImageWidget::saveImage() {
  ...
  if (!fn.isEmpty()) { exportImage().save(fn); }
}
...
void ReceiveRequestDialog::setModel(OptionsModel* model) {
  ...
  if (model) connect(model, SIGNAL(displayUnitChanged(int)), this, SLOT(update()));
...
void ReceiveRequestDialog::update() {
  ...
  if (info.amount)
    html +=
        "<b>" + tr("Amount") + "</b>: " + BitcoinUnits::formatWithUnit(model->getDisplayUnit(), info.amount) + "<br>";
  if (!info.label.isEmpty()) html += "<b>" + tr("Label") + "</b>: " + GUIUtil::HtmlEscape(info.label) + "<br>";
  if (!info.message.isEmpty()) html += "<b>" + tr("Message") + "</b>: " + GUIUtil::HtmlEscape(info.message) + "<br>";
