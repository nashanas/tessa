...
void RPCExecutor::request(const QString& command) {
  ...
  try {
    ...
    UniValue result =
        tableRPC.execute(args[0], RPCConvertValues(args[0], std::vector<std::string>(args.begin() + 1, args.end())));
...
void RPCConsole::setClientModel(ClientModel* model) {
  ...
  if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
    ...
    for (size_t i = 0; i < commandList.size(); ++i) { wordList << commandList[i].c_str(); }
...
void RPCConsole::walletSalvage() { buildParameterlist(SALVAGEWALLET); }

/** Restart wallet with "-rescan" */
void RPCConsole::walletRescan() { buildParameterlist(RESCAN); }

/** Restart wallet with "-zapwallettxes=1" */
void RPCConsole::walletZaptxes1() { buildParameterlist(ZAPTXES1); }

/** Restart wallet with "-zapwallettxes=2" */
void RPCConsole::walletZaptxes2() { buildParameterlist(ZAPTXES2); }

/** Restart wallet with "-upgradewallet" */
void RPCConsole::walletUpgrade() { buildParameterlist(UPGRADEWALLET); }

/** Restart wallet with "-reindex" */
void RPCConsole::walletReindex() { buildParameterlist(REINDEX); }

/** Restart wallet with "-resync" */
void RPCConsole::walletResync() {
  QString resyncWarning =
      tr("This will delete your local blockchain folders and the wallet will synchronize the complete Blockchain from "
         "scratch.<br /><br />");
  ...
  QMessageBox::StandardButton retval =
      QMessageBox::question(this, tr("Confirm resync Blockchain"), resyncWarning,
...
void RPCConsole::clear() {
  ...
  ui->messagesWidget->document()->setDefaultStyleSheet(
      ...
      "td.message { font-family: Courier, Courier New, Lucida Console, monospace; font-size: 12px; } "  // Todo: Remove
                                                                                                        // fixed
                                                                                                        // font-size
  ...
  message(CMD_REPLY,
          (tr("Welcome to the Tessa RPC console.") + "<br>" +
...
void RPCConsole::reject() {
  ...
  if (windowType() != Qt::Widget) QDialog::reject();
}
...
void RPCConsole::setNumBlocks(int count) {
  ...
  if (clientModel) ui->lastBlockTime->setText(clientModel->getLastBlockDate().toString());
}
...
QString RPCConsole::FormatBytes(quint64 bytes) {
  ...
  if (bytes < 1024 * 1024 * 1024) return QString(tr("%1 MB")).arg(bytes / 1024 / 1024);
...
void RPCConsole::showConfEditor() { GUIUtil::openConfigfile(); }

void RPCConsole::peerSelected(const QItemSelection& selected, const QItemSelection& deselected) {
  Q_UNUSED(deselected);

  if (!clientModel || !clientModel->getPeerTableModel() || selected.indexes().isEmpty()) return;
...
void RPCConsole::peerLayoutChanged() {
  ...
  if (!selectedModelIndex.isEmpty()) { selectedRow = selectedModelIndex.first().row(); }
  ...
  if (fUnselect && selectedRow >= 0) { clearSelectedNode(); }

  if (fReselect) { ui->peerWidget->selectRow(detailNodeRow); }
...
void RPCConsole::showBackups() { GUIUtil::showBackups(); }
