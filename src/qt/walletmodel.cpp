...
CAmount WalletModel::getBalance(const CCoinControl* coinControl) const {
  if (coinControl) {
    ...
    for (const COutput& out : vCoins)
      if (out.fSpendable) nBalance += out.tx->vout[out.i].nValue;
...
void WalletModel::updateStatus() {
  ...
  if (cachedEncryptionStatus != newEncryptionStatus) emit encryptionStatusChanged(newEncryptionStatus);
}
...
void WalletModel::updateAddressBook(const QString& address, const QString& label, bool isMine, const QString& purpose,
                                    int status) {
  if (addressTableModel) addressTableModel->updateEntry(address, label, isMine, purpose, status);
}
void WalletModel::updateAddressBook(const QString& pubCoin, const QString& isUsed, int status) {
  if (addressTableModel) addressTableModel->updateEntry(pubCoin, isUsed, status);
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction,
  ...
  if (isAnonymizeOnlyUnlocked()) { return AnonymizeOnlyUnlocked; }
...

  {
    ...
    bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl,
                                              recipients[0].inputType, false);
    transaction.setTransactionFee(nFeeRequired);

    if (!fCreated) {
      if ((total + nFeeRequired) > nBalance) { return SendCoinsReturn(AmountWithFeeExceedsBalance); }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction) {
...

  {
    ...
    if (!wallet->CommitTransaction(*newTx, *keyChange, "tx")) return TransactionCommitFailed;
...
bool WalletModel::isAnonymizeOnlyUnlocked() { return wallet->fWalletUnlockAnonymizeOnly; }
...
static void NotifyAddressBookChanged(WalletModel* walletmodel, CWallet* wallet, const CTxDestination& address,
  ...
  QMetaObject::invokeMethod(walletmodel, "updateAddressBook", Qt::QueuedConnection, Q_ARG(QString, strAddress),
                            Q_ARG(QString, strLabel), Q_ARG(bool, isMine), Q_ARG(QString, strPurpose),
                            Q_ARG(int, status));
}
...
static void NotifyWalletBacked(WalletModel* model, const bool& fSuccess, const string& filename) {
  ...
  QMetaObject::invokeMethod(model, "message", Qt::QueuedConnection, Q_ARG(QString, QString::fromStdString(title)),
                            Q_ARG(QString, QString::fromStdString(message)), Q_ARG(unsigned int, (unsigned int)method));
}
...
void WalletModel::subscribeToCoreSignals() {
  ...
  wallet->NotifyZkpReset.connect(boost::bind(NotifyZkpReset, this));
...
void WalletModel::unsubscribeFromCoreSignals() {
  ...
  wallet->NotifyZkpReset.disconnect(boost::bind(NotifyZkpReset, this));
...
bool WalletModel::isMine(CBitcoinAddress address) { return IsMine(*wallet, address.Get()); }
