 ...
 public:
  ...
  QSize minimumSizeHint() const {
    if (cachedMinimumSizeHint.isEmpty()) {
      ...
      int w = fm.width(
          BitcoinUnits::format(BitcoinUnits::Club, BitcoinUnits::maxMoney(), false, BitcoinUnits::separatorAlways));
  ...
  bool event(QEvent* event) {
    if (event->type() == QEvent::KeyPress || event->type() == QEvent::KeyRelease) {
      ...
      if (keyEvent->key() == Qt::Key_Comma) {
        ...
        QKeyEvent periodKeyEvent(event->type(), Qt::Key_Period, keyEvent->modifiers(), ".", keyEvent->isAutoRepeat(),
                                 keyEvent->count());
