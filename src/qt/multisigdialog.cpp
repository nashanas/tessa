...
MultisigDialog::~MultisigDialog() { delete ui; }

void MultisigDialog::setModel(WalletModel* model) { this->model = model; }
...
void MultisigDialog::addressBookButtonReceiving() {
  ...
  if (model && model->getAddressTableModel()) {
    ...
    if (dlg.exec()) { vle->setText(dlg.getReturnValue()); }
  }
}
...
void MultisigDialog::on_addMultisigButton_clicked() {
  ...
  for (int i = 0; i < ui->addressList->count(); i++) {
    ...
    if (!vle->text().isEmpty()) { keys.push_back(vle->text().toStdString()); }
  }
...
bool MultisigDialog::addMultisig(int m, vector<string> keys) {
  try {
    ...
    if (!createRedeemScript(m, keys, redeem, error)) { throw runtime_error(error.data()); }

    if (::IsMine(*pwalletMain, redeem) == ISMINE_SPENDABLE) {
      throw runtime_error("The wallet already contains this script");
    }

    if (!pwalletMain->AddCScript(redeem)) { throw runtime_error("Failure: address invalid or already exists"); }
    ...
    if (!pwalletMain->AddMultiSig(redeem)) { throw runtime_error("Failure: unable to add address as watch only"); }
...
void MultisigDialog::on_createButton_clicked() {
  ...
  try {
    ...
    if (CoinControlDialog::coinControl->HasSelected()) {
      ...
      for (auto outpoint : vSelected) vUserIn.emplace_back(CTxIn(outpoint));
    ...
    if (validInput) {
      ...
      string error;
      string fee;
      if (!createMultisigTransaction(vUserIn, vUserOut, fee, error)) { throw runtime_error(error); }

      // display status string
      ui->createButtonStatus->setStyleSheet("QTextEdit{ color: black }");

      QString status(
          strprintf("Transaction has successfully created with a fee of %s.\n"
...
bool MultisigDialog::createMultisigTransaction(vector<CTxIn> vUserIn, vector<CTxOut> vUserOut, string& feeStringRet,
  ...
  try {
    ...
    for (CTxIn in : vUserIn) {
      ...
      if (!coins->IsAvailable(in.prevout.n) || coins == nullptr) { continue; }
    ...
    for (CTxIn& in : tx.vin) {
      ...
      for (unsigned int i = 0; i < 50 * (nReq + addresses.size()); i++) { in.scriptSig << INT64_MAX; }
    }
    ...
    for (CTxIn& in : tx.vin) { in.scriptSig.clear(); }
    multisigTx = tx;
  } catch (const runtime_error& e) {
    errorRet = e.what();
    return false;
...
void MultisigDialog::commitMultisigTx() {
  ...
  try {
    ...
    ui->signButtonStatus->setText(
        strprintf("Transaction has been successfully published with transaction ID:\n %s", tx.GetHash().GetHex())
            .c_str());
  } catch (const runtime_error& e) { ui->signButtonStatus->setText(e.what()); }
}
...
bool MultisigDialog::createRedeemScript(int m, vector<string> vKeys, CScript& redeemRet, string& errorRet) {
  try {
    int n = vKeys.size();
    // gather pub keys
    if (n < 1) throw runtime_error("a Multisignature address must require at least one key to redeem");
    if (n < m)
      throw runtime_error(
...
void MultisigDialog::on_addAddressButton_clicked() {
  ...
  addressLabel->setText(QApplication::translate(
      "MultisigDialog", strprintf("Address / Key %i:", ui->addressList->count() + 1).c_str(), 0));
...
void MultisigDialog::on_addInputButton_clicked() {
  ...
  txInputIdLabel->setText(
      QApplication::translate("MultisigDialog", strprintf("%i. Tx Hash: ", ui->inputsList->count() + 1).c_str(), 0));
...
void MultisigDialog::on_addDestinationButton_clicked() {
  ...
  destinationAddressLabel->setText(QApplication::translate(
      "MultisigDialog", strprintf("%i. Address: ", ui->destinationsList->count() + 1).c_str(), 0));
