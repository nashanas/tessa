...
static std::string makeHRef(const std::string& Str) { return "<a href=\"" + Str + "\">" + Str + "</a>"; }

static CAmount getTxIn(const CTransaction& tx) {
  if (tx.IsCoinBase()) return 0;

  CAmount Sum = 0;
  for (unsigned int i = 0; i < tx.vin.size(); i++) Sum += getPrevOut(tx.vin[i].prevout).nValue;
  return Sum;
}
...
static std::string makeHTMLTable(const std::string* pCells, int nRows, int nColumns) {
  ...
  for (int i = 0; i < nRows; i++) Table += makeHTMLTableRow(pCells + i * nColumns, nColumns);
  Table += "</table>";
  return Table;
}
...
static std::string TxToRow(const CTransaction& tx, const CScript& Highlight = CScript(),
  ...
  for (unsigned int j = 0; j < tx.vin.size(); j++) {
    if (tx.IsCoinBase()) {
      ...
      if (PrevOut.scriptPubKey == Highlight) Delta -= PrevOut.nValue;
    }
    if (j + 1 != tx.vin.size()) {
      InAmounts += "<br/>";
      InAddresses += "<br/>";
  ...
  for (unsigned int j = 0; j < tx.vout.size(); j++) {
    ...
    if (Out.scriptPubKey == Highlight) Delta += Out.nValue;
    if (j + 1 != tx.vout.size()) {
      OutAmounts += "<br/>";
      OutAddresses += "<br/>";
    }
  ...
  std::string List[8] = {Prepend, makeHRef(tx.GetHash().GetHex()), InAddresses, InAmounts, OutAddresses, OutAmounts, "",
                         ""};

  int n = sizeof(List) / sizeof(std::string) - 2;

  if (!Highlight.empty()) {
    List[n++] = std::string("<font color=\"") + ((Delta > 0) ? "green" : "red") + "\">" + ValueToString(Delta, true) +
                "</font>";
    *pSum += Delta;
...
CTxOut getPrevOut(const COutPoint& out) {
  CTransaction tx;
  uint256 hashBlock;
  if (GetTransaction(out.hash, tx, hashBlock, true)) return tx.vout[out.n];
  return CTxOut();
}
...
std::string getexplorerBlockHash(int64_t Height) {
  ...
  while (pblockindex->nHeight > Height) pblockindex = pblockindex->pprev;
...
std::string BlockToString(CBlockIndex* pBlock) {
  ...
  std::string BlockContentCells[] = {
      ...
      _("Value Out"),
      ValueToString(OutVolume),
      _("Fees"),
      ValueToString(Fees),
      _("Generated"),
      ValueToString(Generated),
      _("Timestamp"),
      TimeToString(block.nTime),
      _("Difficulty"),
      strprintf("%.4f", GetDifficulty(pBlock)),
      _("Bits"),
      utostr(block.nBits),
      _("Nonce"),
      utostr(block.nNonce),
      ...
      _("Merkle Root"),
      "<pre>" + block.hashMerkleRoot.GetHex() + "</pre>",
  ...
  Content += std::to_string(pBlock->nHeight - 1);
  Content += ">◄&nbsp;</a>";
  Content += _("Block");
  Content += " ";
  Content += std::to_string(pBlock->nHeight);
  ...
  if (block.nHeight > getThirdHardforkBlock())
  {
      ...
      for (int i = 0; i < 2; i++)
      {
          ...
          {
              votes[i].push_back(block.vvotes[i][j].hash.ToString() + ':' + std::to_string(block.vvotes[i][j].n));
          }
      }
...
std::string TxToString(uint256 BlockHash, const CTransaction& tx) {
  ...
  if (tx.IsCoinBase()) {
    std::string InputsContentCells[] = {"0", "coinbase", "-", ValueToString(Output)};
  ...
  if (iter != mapBlockIndex.end()) {
    ...
    Labels[0 * 2 + 1] = makeHRef(std::to_string(pIndex->nHeight));
...
BlockExplorer::BlockExplorer(QWidget* parent)
    : QMainWindow(parent), ui(new Ui::BlockExplorer), m_NeverShown(true), m_HistoryIndex(0) {
  ui->setupUi(this);
...
BlockExplorer::~BlockExplorer() { delete ui; }
...
void BlockExplorer::goTo(const QString& query) {
  if (switchTo(query)) {
    ...
    while (m_History.size() > m_HistoryIndex + 1) m_History.pop_back();
...
void BlockExplorer::onSearch() { goTo(ui->searchBox->text()); }

void BlockExplorer::setBlock(CBlockIndex* pBlock) { setContent(BlockToString(pBlock)); }
...
void BlockExplorer::setContent(const std::string& Content) {
  QString CSS =
      ...
      "{color:#5B4C7C; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: "
      "#5B4C7C;}\n td.d0 {font-weight: bold; color:#f8f6f6;}\n h2, h3 { white-space:nowrap; color:#5B4C7C;}\n a { "
      "color:#88f6f6; text-decoration:none; }\n a.nav {color:#5B4C7C;}\n";
  QString FullContent =
      "<html><head><style type=\"text/css\">" + CSS + "</style></head>" + "<body>" + Content.c_str() + "</body></html>";
