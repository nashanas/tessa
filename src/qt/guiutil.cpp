...
bool parseBitcoinURI(const QUrl& uri, SendCoinsRecipient* out) {
  ...
  if (rv.address.endsWith("/")) { rv.address.truncate(rv.address.length() - 1); }
  rv.amount = 0;
...
QString HtmlEscape(const QString& str, bool fMultiLine) {
  ...
  if (fMultiLine) { escaped = escaped.replace("\n", "<br>\n"); }
  return escaped;
}
...
QString getEntryData(QAbstractItemView* view, int column, int role) {
  if (!view || !view->selectionModel()) return QString();
  QModelIndexList selection = view->selectionModel()->selectedRows(column);
...
QString getSaveFileName(QWidget* parent, const QString& caption, const QString& dir, const QString& filter,
  ...
  if (filter_re.exactMatch(selectedFilter)) { selectedSuffix = filter_re.cap(1); }
  ...
  if (selectedSuffixOut) { *selectedSuffixOut = selectedSuffix; }
  return result;
}
...
// happen before the QApplication is created.
  ...
  if (floor(NSAppKitVersionNumber) > NSAppKitVersionNumber10_8) {
    if (floor(NSAppKitVersionNumber) <= NSAppKitVersionNumber10_9) /* On a 10.9 - 10.9.x system */
...
bool ToolTipToRichTextFilter::eventFilter(QObject* obj, QEvent* evt) {
  if (evt->type() == QEvent::ToolTipChange) {
    ...
    if (tooltip.size() > size_threshold && !tooltip.startsWith("<qt")) {
      ...
      if (!Qt::mightBeRichText(tooltip)) tooltip = HtmlEscape(tooltip, true);
...
int TableViewLastColumnResizingFixer::getColumnsWidth() {
  ...
  for (int i = 0; i < columnCount; i++) { nColumnsWidthSum += tableView->horizontalHeader()->sectionSize(i); }
...
void TableViewLastColumnResizingFixer::on_sectionResized(int logicalIndex, int oldSize, int newSize) {
  ...
  if (newSize > remainingWidth) { resizeColumn(logicalIndex, remainingWidth); }
}
 ...
 */
DHMSTableWidgetItem::DHMSTableWidgetItem(const int64_t seconds) : QTableWidgetItem(), value(seconds) {
...
bool DHMSTableWidgetItem::operator<(QTableWidgetItem const& item) const {
  DHMSTableWidgetItem const* rhs = dynamic_cast<DHMSTableWidgetItem const*>(&item);

  if (!rhs) return QTableWidgetItem::operator<(item);
...
bool GetStartOnSystemStartup() {
  ...
  while (!optionFile.eof()) {
    ...
    if (line.find("Hidden") != std::string::npos && line.find("true") != std::string::npos) return false;
  }
  optionFile.close();
...
bool SetStartOnSystemStartup(bool fAutoStart) {
  if (!fAutoStart)
    ...
    if (readlink("/proc/self/exe", pszExePath, sizeof(pszExePath) - 1) == -1) return false;
...
QString loadStyleSheet() {
  ...
  QFile qFile(cssName);
  if (qFile.open(QFile::ReadOnly)) { styleSheet = QLatin1String(qFile.readAll()); }
...
QString formatDurationStr(int secs) {
  ...
  if (seconds || (!days && !hours && !mins)) strList.append(QString(QObject::tr("%1 s")).arg(seconds));
