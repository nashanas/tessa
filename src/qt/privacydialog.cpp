...
PrivacyDialog::PrivacyDialog(QWidget* parent)
  ...
  ui->labelZKPSyncStatus->setText("(" + tr("out of sync") + ")");
  ...
  // Set labels/buttons depending on zeroSPORK status
  updateZeroSPORKStatus();
}

PrivacyDialog::~PrivacyDialog() { delete ui; }
...
void PrivacyDialog::setModel(WalletModel* walletModel) {
  ...
  if (walletModel && walletModel->getOptionsModel()) {
    ...
    connect(walletModel,
            ...
            this, SLOT(setBalance(CAmount, CAmount, CAmount, CAmount, CAmount, CAmount, CAmount, CAmount, CAmount)));
...
void PrivacyDialog::on_pushButtonMintZKP_clicked() {
  ...
  if (encStatus == walletModel->Locked) {
    WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_ZKP, true));
    if (!ctx.isValid()) {
  ...
  ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " ZKP...");
...
void PrivacyDialog::on_pushButtonMintReset_clicked() {
  ...
  double fDuration = (double)(GetTimeMillis() - nTime) / 1000.0;
  ui->TEMintStatus->setPlainText(QString::fromStdString(strResetMintResult) + tr("Duration: ") +
                                 QString::number(fDuration) + tr(" sec.\n"));
  ui->TEMintStatus->repaint();
  ui->TEMintStatus->verticalScrollBar()->setValue(
      ui->TEMintStatus->verticalScrollBar()->maximum());  // Automatically scroll to end of text
...
void PrivacyDialog::on_pushButtonSpentReset_clicked() {
  ...
  double fDuration = (double)(GetTimeMillis() - nTime) / 1000.0;
  ui->TEMintStatus->setPlainText(QString::fromStdString(strResetSpentResult) + tr("Duration: ") +
                                 QString::number(fDuration) + tr(" sec.\n"));
  ui->TEMintStatus->repaint();
  ui->TEMintStatus->verticalScrollBar()->setValue(
      ui->TEMintStatus->verticalScrollBar()->maximum());  // Automatically scroll to end of text
...
void PrivacyDialog::on_pushButtonSpendZKP_clicked() {
  ...
  if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
    WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_ZKP, true));
    if (!ctx.isValid()) {
      // Unlock wallet was cancelled
      return;
    }
    // Wallet is unlocked now, sedn ZKP
    sendZKP();
    return;
  }
  // Wallet already unlocked or not encrypted at all, send ZKP
  sendZKP();
}
...
void PrivacyDialog::sendZKP() {
  ...
  double dAmount = ui->ZKPpayAmount->text().toDouble();
  ...
  if (!fWholeNumber) dzFee = 1.0 - (dAmount - floor(dAmount));
  ...
  QMessageBox::StandardButton retval = QMessageBox::question(
      this, tr("Confirm send coins"), strQuestionString, QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
  ...
  int64_t nTime = GetTimeMillis();
  ui->TEMintStatus->setPlainText(
      tr("Spending Zerocoin.\nComputationally expensive, might need several minutes depending on the selected Security "
  ...
  if (!fSuccess) {
    ...
    if (nNeededSpends > nMaxSpends) {
      ...
      QMessageBox::warning(this, tr("Spend Zerocoin"), strStatusMessage.toStdString().c_str(), QMessageBox::Ok,
                           QMessageBox::Ok);
      ui->TEMintStatus->setPlainText(tr("Spend Zerocoin failed with status = ") +
                                     QString::number(receipt.GetStatus(), 10) + "\n" +
  ...
  for (CZerocoinSpend spend : receipt.GetSpends()) {
    strStats += tr("ZKP Spend #: ") + QString::number(nCount) + ", ";
...
void PrivacyDialog::coinControlClipboardQuantity() { GUIUtil::setClipboard(ui->labelCoinControlQuantity->text()); }
...
void PrivacyDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance,
                               const CAmount& immatureBalance, const CAmount& zerocoinBalance,
  ...
  std::vector<CMintMeta> vMints = pwalletMain->zkpTracker->GetMints(false);
  ...
  for (const auto& denom : libzerocoin::zerocoinDenomList) {
    ...
    strUnconfirmed = "";
    if (nUnconfirmed) { strUnconfirmed += QString::number(nUnconfirmed) + QString(" unconf. "); }
    if (nImmature) { strUnconfirmed += QString::number(nImmature) + QString(" immature "); }
  ...
  ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply() / COIN) +
                                  QString(" <b>ZKP </b> "));
  ...
  if (walletModel && walletModel->getOptionsModel()) {
    ...
    if (currentBalance != -1)
      setBalance(currentBalance, currentUnconfirmedBalance, currentImmatureBalance, currentZerocoinBalance,
                 currentUnconfirmedZerocoinBalance, currentImmatureZerocoinBalance, currentWatchOnlyBalance,
                 currentWatchUnconfBalance, currentWatchImmatureBalance);
  }
}

void PrivacyDialog::showOutOfSyncWarning(bool fShow) { ui->labelZKPSyncStatus->setVisible(fShow); }
...
void PrivacyDialog::updateZeroSPORKStatus() {
  ...
  if (GetAdjustedTime() > gSporkManager.GetSporkValue(SporkID::SPORK_ZEROCOIN_MAINTENANCE_MODE)) {
    ...
    // Spend ZKP
    ui->pushButtonSpendZKP->setEnabled(false);
    ui->pushButtonSpendZKP->setToolTip(tr("ZKP is currently disabled due to maintenance."));
  } else {
    // Mint ZKP
