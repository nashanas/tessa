...
/* Window elements init */
  ...
  if (!enableWallet) { ui->tabWidget->removeTab(ui->tabWidget->indexOf(ui->tabWallet)); }
  ...
  foreach (const QString& langStr, translations.entryList()) {
    ...
    if (langStr.contains("_")) {
      /** display language strings as "native language - native country (locale name)", e.g. "Deutsch - Deutschland
       * (de)" */
      ui->lang->addItem(locale.nativeLanguageName() + QString(" - ") + locale.nativeCountryName() + QString(" (") +
                            langStr + QString(")"),
                        QVariant(langStr));
    } else {
...
void OptionsDialog::setModel(OptionsModel* model) {
  ...
  if (model) {
    ...
    if (model->isRestartRequired()) showRestartWarning(true);
...
void OptionsDialog::enableOkButton() {
  ...
  if (fProxyIpValid) setOkButtonState(true);
}
...
void OptionsDialog::on_resetButton_clicked() {
  if (model) {
    ...
    QMessageBox::StandardButton btnRetVal =
        QMessageBox::question(this, tr("Confirm options reset"),
...
bool OptionsDialog::eventFilter(QObject* object, QEvent* event) {
  if (event->type() == QEvent::FocusOut) {
    if (object == ui->proxyIp || object == ui->proxyPort) { emit proxyIpChecks(ui->proxyIp, ui->proxyPort); }
  }
