...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit) {
      ...
      if (CBitcoinAddress(rec->address).IsValid()) {
        ...
        if (wallet->mapAddressBook.count(address)) {
          ...
          QString addressOwned =
              (::IsMine(*wallet, address) == ISMINE_SPENDABLE) ? tr("own address") : tr("watch-only");
  ...
  if (wtx.IsCoinBase() && rec->credit == 0) {
    ...
    if (fAllFromMe) {
      if (fAllFromMe == ISMINE_WATCH_ONLY) strHTML += "<b>" + tr("From") + ":</b> " + tr("watch-only") + "<br>";
      ...
      for (const CTxOut& txout : wtx.vout) {
        ...
        if ((toSelf == ISMINE_SPENDABLE) && (fAllFromMe == ISMINE_SPENDABLE)) continue;
  ...
  // Message from normal club:URI (club:XyZ...?message=example)
  foreach (const auto& r, wtx.vOrderForm)
    if (r.first == "Message")
  ...
  if (wtx.IsCoinBase()) {
    ...
    strHTML += "<br>" +
               tr("Generated coins must mature %1 blocks before they can be spent. When you generated this block, it "
                  "was broadcast to the network to be added to the block chain. If it fails to get into the chain, its "
                  "state will change to \"not accepted\" and it won't be spendable. This may occasionally happen if "
