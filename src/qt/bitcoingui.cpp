...
BitcoinGUI::BitcoinGUI(const NetworkStyle* networkStyle, QWidget* parent)
  ...
  if (curStyle == "QWindowsStyle" || curStyle == "QWindowsXPStyle") {
    progressBar->setStyleSheet(
        "QProgressBar { background-color: #F8F8F8; border: 1px solid grey; border-radius: 7px; padding: 1px; "
        "text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 "
        "#00CCFF, stop: 1 #33CCFF); border-radius: 7px; margin: 0px; }");
  }
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle) {
  ...
  sendCoinsAction->setStatusTip(tr("Send coins to a Club address"));
  ...
  aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About Club Core"), this);
  aboutAction->setStatusTip(tr("Show information about Club Core"));
  aboutAction->setMenuRole(QAction::AboutRole);
  aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
  aboutQtAction->setStatusTip(tr("Show information about Qt"));
  aboutQtAction->setMenuRole(QAction::AboutQtRole);
  optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
  optionsAction->setStatusTip(tr("Modify configuration options for Club"));
  ...
  signMessageAction->setStatusTip(tr("Sign messages with your Club addresses to prove you own them"));
  verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
  verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Club addresses"));
  ...
  openInfoAction =
      new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Information"), this);
  ...
  openAction->setStatusTip(tr("Open a Club: URI or payment request"));
  ...
  showHelpMessageAction->setStatusTip(
      tr("Show the Club Core help message to get a list with possible Club command-line options"));
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle) {
  ...
  QString toolTip = tr("Club Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::setNumConnections(int count) {
  ...
  labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Club network", "", count));
}
...
void BitcoinGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret) {
  QString strTitle = tr("Club Core");  // default title
  ...
  if (style & CClientUIInterface::MODAL) {
    ...
    if (!(buttons = (QMessageBox::StandardButton)(style & CClientUIInterface::BTN_MASK))) buttons = QMessageBox::Ok;
...
void BitcoinGUI::closeEvent(QCloseEvent* event) {
  ...
  if (clientModel && clientModel->getOptionsModel()) {
    if (!clientModel->getOptionsModel()->getMinimizeOnClose()) { QApplication::quit(); }
  }
#endif
...
void BitcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type,
  ...
  if (!bdisableSystemnotifications) {
    ...
    message((amount) < 0
                ? (pwalletMain->fMultiSendNotify == true ? tr("Sent MultiSend transaction") : tr("Sent transaction"))
...
void BitcoinGUI::dropEvent(QDropEvent* event) {
  if (event->mimeData()->hasUrls()) {
    foreach (const QUrl& uri, event->mimeData()->urls()) { emit receivedURI(uri.toString()); }
  }
...
bool BitcoinGUI::eventFilter(QObject* object, QEvent* event) {
  ...
  if (event->type() == QEvent::StatusTip) {
    ...
    if (progressBarLabel->isVisible() || progressBar->isVisible()) return true;
  }
...
void BitcoinGUI::setEncryptionStatus(int status) {
  switch (status) {
    ...
    case WalletModel::UnlockedForAnonymizationOnly:
      labelEncryptionIcon->show();
      labelEncryptionIcon->setIcon(QIcon(":/icons/lock_open").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
      labelEncryptionIcon->setToolTip(
          tr("Wallet is <b>encrypted</b> and currently <b>unlocked</b> for anonymization and staking only"));
      encryptWalletAction->setChecked(true);
      changePassphraseAction->setEnabled(true);
...
static bool ThreadSafeMessageBox(BitcoinGUI* gui, const std::string& message, const std::string& caption,
                                 unsigned int style) {
  ...
  QMetaObject::invokeMethod(gui, "message", modal ? GUIUtil::blockingGUIThreadConnection() : Qt::QueuedConnection,
                            ...
                            Q_ARG(QString, QString::fromStdString(message)), Q_ARG(unsigned int, style),
                            Q_ARG(bool*, &ret));
  return ret;
}
...
void BitcoinGUI::handleRestart(QStringList args) {
  if (!ShutdownRequested()) emit requestedRestart(args);
}

UnitDisplayStatusBarControl::UnitDisplayStatusBarControl() : optionsModel(0), menu(0) {
  createContextMenu();
...
void UnitDisplayStatusBarControl::onMenuSelection(QAction* action) {
  if (action) { optionsModel->setDisplayUnit(action->data()); }
}
