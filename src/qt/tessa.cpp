...
static QString GetLangTerritory() {
  ...
  if (!lang_territory_qsettings.isEmpty()) lang_territory = lang_territory_qsettings;
...
void BitcoinApplication::requestShutdown() {
  ...
  if (walletModel) delete walletModel;
  walletModel = nullptr;

  if (clientModel) delete clientModel;
  clientModel = nullptr;
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
  ...
  //   http://qt-project.org/doc/qt-5/qmetatype.html#qRegisterMetaType) IMPORTANT if it is no longer a typedef use the
  //   normal variant above
