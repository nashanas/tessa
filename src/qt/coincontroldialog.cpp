...
// click on header
  ...
  QSettings settings;
  if (settings.contains("nCoinControlMode") && !settings.value("nCoinControlMode").toBool()) ui->radioTreeMode->click();
...
void CoinControlDialog::buttonBoxClicked(QAbstractButton* button) {
  if (ui->buttonBox->buttonRole(button) == QDialogButtonBox::AcceptRole) done(QDialog::Accepted);  // closes the dialog
}
...
void CoinControlDialog::buttonSelectAllClicked() {
  ...
  if (state == Qt::Unchecked) coinControl->UnSelectAll();  // just to be sure
  CoinControlDialog::updateLabels(model, this);
  updateDialogLabels();
}
...
void CoinControlDialog::clipboardBytes() { GUIUtil::setClipboard(ui->labelCoinControlBytes->text().replace("~", "")); }
...
QString CoinControlDialog::getPriorityLabel(double dPriority, double mempoolEstimatePriority) {
  ...
  if (dPriorityMedium <= 0) dPriorityMedium = AllowFreeThreshold();  // not enough data, back to hard-coded
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog) {
  ...
  foreach (const CAmount& amount, CoinControlDialog::payAmounts) {
    ...
    if (amount > 0) {
      ...
      if (txout.IsDust(::minRelayTxFee)) fDust = true;
    }
  }
  ...
  if (nQuantity > 0) {
    ...
    nBytes = nBytesInputs +
             ((CoinControlDialog::payAmounts.size() > 0
                   ? CoinControlDialog::payAmounts.size() + max(1, CoinControlDialog::nSplitBlockDummy)
                   : 2) *
              34) +
    ...
    dPriority = dPriorityInputs /
                (nBytes - nBytesInputs +
                 (nQuantityUncompressed * 29));  // 29 = 180 - 151 (uncompressed public keys are over the limit. max 151
    ...
    if (dPriorityNeeded <= 0) dPriorityNeeded = AllowFreeThreshold();  // not enough data, back to hard-coded
    fAllowFree = (dPriority >= dPriorityNeeded);

    if (fSendFreeTransactions)
      if (fAllowFree && nBytes <= MAX_FREE_TRANSACTION_CREATE_SIZE) nPayFee = 0;
  ...
  int nDisplayUnit = BitcoinUnits::Club;
  ...
  toolTip1 += tr("This means a fee of at least %1 per kB is required.")
                  .arg(BitcoinUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK())) +
              "<br /><br />";
...
void CoinControlDialog::updateView() {
  if (!model || !model->getOptionsModel() || !model->getAddressTableModel()) return;
  ...
  for (auto coins : mapCoins) {
    ...
    for (const COutput& out : coins.second) {
      ...
      if (fMultisigEnabled && !fMultiSigUTXO) continue;
      int nInputSize = 0;
      ...
      if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
        ...
        // if listMode or change => show Club address. In tree mode, address is not shown again for direct wallet
        // address outputs
      ...
      // set checkbox
      if (coinControl->IsSelected(txhash, out.i)) itemOutput->setCheckState(COLUMN_CHECKBOX, Qt::Checked);
    }
