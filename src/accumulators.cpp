...
int GetChecksumHeight(uint32_t nChecksum, CoinDenomination denomination) {
  ...
  while (pindex) {
    if (ParseChecksum(pindex->nAccumulatorCheckpoint, denomination) == nChecksum) return pindex->nHeight;

    // Skip forward in groups of ACC_BLOCK_INTERVAL blocks since checkpoints only change every ACC_BLOCK_INTERVAL blocks
    if (pindex->nHeight % ACC_BLOCK_INTERVAL == 0) {
      if (pindex->nHeight + ACC_BLOCK_INTERVAL > chainActive.Height()) return 0;
      pindex = chainActive[pindex->nHeight + ACC_BLOCK_INTERVAL];
      continue;
    }
...
bool EraseAccumulatorValues(const uint256& nCheckpointErase, const uint256& nCheckpointPrevious) {
  for (auto& denomination : zerocoinDenomList) {
    ...
    if (nChecksumErase == nChecksumPrevious) continue;

    if (!EraseChecksum(nChecksumErase)) return false;
  }
...
bool EraseCheckpoints(int nStartHeight, int nEndHeight) {
  if (chainActive.Height() < nStartHeight) return false;
  ...
  for (auto denom : zerocoinDenomList) listCheckpointsPrev.emplace_back(ParseChecksum(nCheckpointPrev, denom));
...
bool InitializeAccumulators(const int nHeight, int& nHeightCheckpoint, AccumulatorMap& mapAccumulators) {
  if (nHeight < Params().Zerocoin_StartHeight()) return error("%s: height is below zerocoin activated", __func__);

  if (nHeight >= Params().Zerocoin_StartHeight()) {
    // 2*ACC_BLOCK_INTERVAL after start is when the new checkpoints will be in the block,
    // so don't need to load hard checkpoints
    if (nHeight <= Params().Zerocoin_StartHeight() + 2 * ACC_BLOCK_INTERVAL) {
...
bool CalculateAccumulatorCheckpoint(int nHeight, uint256& nCheckpoint, AccumulatorMap& mapAccumulators) {
  ...
  if (nHeight % ACC_BLOCK_INTERVAL != 0) {
  ...
  // Accumulate all coins over the last ten blocks that havent been accumulated (height - 2*ACC_BLOCK_INTERVAL through
  // height - 11)
...
#warning "Check this"
  ...
  while (pindex->nHeight < nHeight - ACC_BLOCK_INTERVAL) {
    ...
    CBlock block;
    if (!ReadBlockFromDisk(block, pindex)) return error("%s: failed to read block from disk", __func__);
...
bool ValidateAccumulatorCheckpoint(const CBlock& block, CBlockIndex* pindex, AccumulatorMap& mapAccumulators) {
  ...
  // checkpoints It is VERY IMPORTANT that when this is being run and height < v2_start, then ZKP need to be disabled
  // at the same time!!
  ...
  if (pindex->nHeight % ACC_BLOCK_INTERVAL == 0) {
    ...
    if (nCheckpointCalculated != block.nAccumulatorCheckpoint) {
      LogPrintf("%s: block=%d calculated: %s\n block: %s\n", __func__, pindex->nHeight, nCheckpointCalculated.GetHex(),
                block.nAccumulatorCheckpoint.GetHex());
  ...
  if (block.nAccumulatorCheckpoint != pindex->pprev->nAccumulatorCheckpoint)
    return error("%s : new accumulator checkpoint generated on a block that is not multiple of ACC_BLOCK_INTERVAL",
                 __func__);
...
void RandomizeSecurityLevel(int& nSecurityLevel) {
  // security level: this is an important prevention of tracing the coins via timing. Security level represents how many
  // checkpoints of accumulated coins are added *beyond* the checkpoint that the mint being spent was added too. If each
  // spend added the exact same amounts of checkpoints after the mint was accumulated, then you could know the range of
...
int AddBlockMintsToAccumulator(const libzerocoin::PublicCoin& coin, const int nHeightMintAdded,
                               const CBlockIndex* pindex, libzerocoin::Accumulator* accumulator, bool isWitness) {
  ...
  if (pindex->MintedDenomination(coin.getDenomination())) {
    ...
    for (const PublicCoin& pubcoin : listPubcoins) {
      if (pubcoin.getDenomination() != coin.getDenomination()) continue;

      if (isWitness && pindex->nHeight == nHeightMintAdded && pubcoin.getValue() == coin.getValue()) continue;
...
bool GetAccumulatorValue(int& nHeight, const libzerocoin::CoinDenomination denom, CBigNum& bnAccValue) {
  if (nHeight > chainActive.Height()) return error("%s: height %d is more than active chain height", __func__, nHeight);

  // Every situation except for about 2*ACC_BLOCK_INTERVAL blocks should use this method
...
bool GenerateAccumulatorWitness(const PublicCoin& coin, Accumulator& accumulator, AccumulatorWitness& witness,
  ...
  uint256 hashBlock;
  if (!GetTransaction(txid, txMinted, hashBlock)) return error("%s failed to read tx", __func__);
  ...
  CBlockIndex* pindex = chainActive[nHeightCheckpoint - ACC_BLOCK_INTERVAL];
  int nChainHeight = chainActive.Height();
  int nHeightStop = nChainHeight % ACC_BLOCK_INTERVAL;
  nHeightStop = nChainHeight - nHeightStop - 2 * ACC_BLOCK_INTERVAL;  // at least two checkpoints deep

  // If looking for a specific checkpoint
  if (pindexCheckpoint) nHeightStop = pindexCheckpoint->nHeight - ACC_BLOCK_INTERVAL;
  ...
  while (pindex) {
    ...
    if (pindex->nHeight >= nHeightStop || fSecurityLevelSatisfied) {
      bnAccValue = 0;
      uint256 nCheckpointSpend = chainActive[pindex->nHeight + ACC_BLOCK_INTERVAL]->nAccumulatorCheckpoint;
  ...
  if (!witness.VerifyWitness(accumulator, coin)) { return error("%s: failed to verify witness", __func__); }

  // A certain amount of accumulated coins are required
  if (nMintsAdded < Params().Zerocoin_RequiredAccumulation()) {
    strError =
        strprintf("Less than %d mints added, unable to create spend", Params().Zerocoin_RequiredAccumulation()).c_str();
...
map<CoinDenomination, int> GetMintMaturityHeight() {
  ...
  for (auto denom : libzerocoin::zerocoinDenomList) mapDenomMaturity.insert(make_pair(denom, make_pair(0, 0)));
  ...
  int nMinimumMaturityHeight = nConfirmedHeight - (nConfirmedHeight % ACC_BLOCK_INTERVAL);
  ...
  while (pindex && pindex->nHeight > Params().Zerocoin_StartHeight()) {
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
      ...
      if (mapDenomMaturity.at(denom).first < Params().Zerocoin_RequiredAccumulation()) {
        mapDenomMaturity.at(denom).first +=
  ...
  for (auto denom : libzerocoin::zerocoinDenomList) mapRet.insert(make_pair(denom, mapDenomMaturity.at(denom).second));
