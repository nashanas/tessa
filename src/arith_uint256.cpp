...
template <unsigned int BITS> base_uint<BITS>& base_uint<BITS>::operator/=(const base_uint& b) {
  ...
  *this = 0;                    // the quotient.
  ...
  if (div_bits == 0) throw std::runtime_error("Division by zero");
...
template <unsigned int BITS> void base_uint<BITS>::SetHex(const std::string& str) { SetHex(str.c_str()); }

template <unsigned int BITS> std::string base_uint<BITS>::ToString() const { return (GetHex()); }
...
uint64_t GetHash(const arith_uint256& ref, const arith_uint256& salt) {
  uint32_t a, b, c;
  a = b = c = 0xdeadbeef + (32 << 2);
  a += ref.get_byte(0) ^ salt.get_byte(0);
  b += ref.get_byte(1) ^ salt.get_byte(1);
  c += ref.get_byte(2) ^ salt.get_byte(2);
  HashMix(a, b, c);
  a += ref.get_byte(3) ^ salt.get_byte(3);
  b += ref.get_byte(4) ^ salt.get_byte(4);
  c += ref.get_byte(5) ^ salt.get_byte(5);
  HashMix(a, b, c);
  a += ref.get_byte(6) ^ salt.get_byte(6);
  b += ref.get_byte(7) ^ salt.get_byte(7);
  HashFinal(a, b, c);
