...
bool CZMQAbstractPublishNotifier::Initialize(void *pcontext)
{
    ...
    {
        LogPrint(ClubLog::ZMQ, "zmq: Reusing socket for address %s\n", address);
...
void CZMQAbstractPublishNotifier::Shutdown()
{
    ...
    if (count == 1)
    {
        LogPrint(ClubLog::ZMQ, "Close socket at address %s\n", address);
        int linger = 0;
...
bool CZMQPublishHashBlockNotifier::NotifyBlock(const CBlockIndex *pindex)
{
    ...
    LogPrint(ClubLog::ZMQ, "zmq: Publish hashblock %s\n", hash.GetHex());
    char data[32];
...
bool CZMQPublishHashTransactionNotifier::NotifyTransaction(const CTransaction &transaction)
{
    ...
    LogPrint(ClubLog::ZMQ, "zmq: Publish hashtx %s\n", hash.GetHex());
    char data[32];
    for (unsigned int i = 0; i < 32; i++)
        data[31 - i] = hash.begin()[i];
...
{
    uint256 hash = transaction.GetHash();
    LogPrint(ClubLog::ZMQ, "zmq: Publish hashtxlock %s\n", hash.GetHex());
    char data[32];
    for (unsigned int i = 0; i < 32; i++)
        data[31 - i] = hash.begin()[i];
...
{
    LogPrint(ClubLog::ZMQ, "zmq: Publish rawblock %s\n", pindex->GetBlockHash().GetHex());
...
{
    uint256 hash = transaction.GetHash();
    LogPrint(ClubLog::ZMQ, "zmq: Publish rawtx %s\n", hash.GetHex());
    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
    ss << transaction;
...
{
    uint256 hash = transaction.GetHash();
    LogPrint(ClubLog::ZMQ, "zmq: Publish rawtxlock %s\n", hash.GetHex());
    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
    ss << transaction;
