...
static bool AppInitRawTx(int argc, char* argv[]) {
  ...
  if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help")) {
    ...
    std::string strUsage =
        _("Tessa Core tessa-tx utility version") + " " + FormatFullVersion() + "\n\n" + _("Usage:") + "\n" +
    ...
    strUsage += HelpMessageOpt("sign=SIGHASH-FLAGS",
                               _("Add zero or more signatures to transaction") + ". " +
                                   _("This command requires JSON registers:") + _("prevtxs=JSON object") + ", " +
...
static void MutateTxVersion(CMutableTransaction& tx, const string& cmdVal) {
  ...
  if (newVersion < 1 || newVersion > CTransaction::CURRENT_VERSION) throw runtime_error("Invalid TX version requested");
...
static int CommandLineRawTx(int argc, char* argv[]) {
  ...
  if (strPrint != "") { fprintf((nRet == 0 ? stdout : stderr), "%s\n", strPrint.c_str()); }
  return nRet;
}
