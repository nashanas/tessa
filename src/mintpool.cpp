...
void CMintPool::Add(const pair<uint256, uint32_t>& pMint, bool fVerbose) {
  insert(pMint);
  if (pMint.second > nCountLastGenerated) nCountLastGenerated = pMint.second;

  if (fVerbose)
    LogPrintf("%s : add %s count %d to mint pool\n", __func__, pMint.first.GetHex().substr(0, 6), pMint.second);
}

bool CMintPool::Has(const CBigNum& bnValue) { return static_cast<bool>(count(GetPubCoinHash(bnValue))); }
...
bool SortSmallest(const pair<uint256, uint32_t>& a, const pair<uint256, uint32_t>& b) { return a.second < b.second; }

std::list<pair<uint256, uint32_t> > CMintPool::List() {
  list<pair<uint256, uint32_t> > listMints;
  for (auto pMint : *(this)) { listMints.emplace_back(pMint); }
