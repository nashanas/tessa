...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<CBlockIndex*>& vBlocks,
  ...
  // last linked block we have in common with this peer. The +1 is so we can detect stalling, namely if we would be able
  // to download that next block if the window were 1 larger.
...
bool CheckTransaction(const CTransaction& tx, bool fZerocoinActive, CValidationState& state) {
  ...
  if (fZerocoinActive) {
    if (nZCSpendCount > Params().Zerocoin_MaxSpendsPerTransaction())
      return state.DoS(
          100, error("CheckTransaction() : there are more zerocoin spends than are allowed in one transaction"));
  ...
  for (const CTxIn& txin : tx.vin) {
    ...
    if (!txin.scriptSig.IsZerocoinSpend()) vInOutPoints.insert(txin.prevout);
  }
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree,
...

  {
    ...
    if (tx.IsZerocoinSpend()) {
      ...
      for (const CTxIn& txIn : tx.vin) {
        ...
        if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), uint256()))
          return state.Invalid(
    ...
    if (!tx.IsZerocoinSpend()) {
      ...
      if (nSigOps > nMaxSigOps)
        return state.DoS(
            0, error("AcceptToMemoryPool : too many sigops %s, %d > %d", hash.ToString(), nSigOps, nMaxSigOps),
    ...
    if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true)) {
      return error(
          "AcceptToMemoryPool: : BUG! PLEASE REPORT THIS! ConnectInputs failed against MANDATORY but not STANDARD "
          "flags %s",
          hash.ToString());
    }
  ...
  if (tx.IsZerocoinSpend()) mapZerocoinspends[tx.GetHash()] = GetAdjustedTime();
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree,
  ...
  // Coinbase is only valid in a block, not as a loose transaction
  if (tx.IsCoinBase())
    return state.DoS(100, error("AcceptableInputs: : coinbase as individual tx"), REJECT_INVALID, "coinbase");
  ...
  // is it already in the memory pool?
  uint256 hash = tx.GetHash();
  if (pool.exists(hash)) return false;
...

  {
    ...
    CAmount nValueIn = 0;
    {
      ...
      // are the actual inputs available?
      if (!view.HaveInputs(tx))
        return state.Invalid(error("AcceptableInputs : inputs already spent"), REJECT_DUPLICATE,
                             "bad-txns-inputs-spent");
    ...
    //        {
    //            return error("AcceptableInputs: : BUG! PLEASE REPORT THIS! ConnectInputs failed against MANDATORY but
    //            not STANDARD flags %s", hash.ToString());
    //        }
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
      ...
      if (coins->IsCoinBase() || coins->IsCoinStake()) {
        if (nSpendHeight - coins->nHeight < Params().COINBASE_MATURITY())
          return state.Invalid(error("CheckInputs() : tried to spend coinbase at depth %d, coinstake=%d",
    ...
    if (!tx.IsCoinStake()) {
      if (nValueIn < tx.GetValueOut())
        return state.DoS(100,
                         error("CheckInputs() : %s value in (%s) < value out (%s)", tx.GetHash().ToString(),
      ...
      for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (pvChecks) {
          ...
          if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
            ...
            // non-upgraded nodes.
            CScriptCheck check(*coins, tx, i, flags & ~STANDARD_NOT_MANDATORY_VERIFY_FLAGS, cacheStore);
            if (check())
              return state.Invalid(
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view,
                  bool fJustCheck, bool fAlreadyChecked) {
  AssertLockHeld(cs_main);
  ...
  for (unsigned int i = 0; i < block.vtx.size(); i++) {
    ...
    CTxUndo undoDummy;
    if (i > 0) { blockundo.vtxundo.push_back(CTxUndo()); }
  ...
  */
  ...
  for (const uint256& txid : vSpendsInBlock) {
    ...
    if (it != mapZerocoinspends.end()) mapZerocoinspends.erase(it);
  }
...
bool static DisconnectTip(CValidationState& state) {
  ...
  for (const CTransaction& tx : block.vtx) { SyncWithWallets(tx, nullptr); }
  return true;
}
...
bool static ConnectTip(CValidationState& state, CBlockIndex* pindexNew, CBlock* pblock, bool fAlreadyChecked) {
  ...
  CBlock block;
  if (!pblock) {
    if (!ReadBlockFromDisk(block, pindexNew)) return state.Abort("Failed to read block");
    pblock = &block;
  }
  // Apply the block atomically to the chain state.
  int64_t nTime2 = GetTimeMicros();
  ...
  // to conflicted:
  for (const CTransaction& tx : txConflicted) { SyncWithWallets(tx, nullptr); }
  // ... and about transactions that got confirmed:
  for (const CTransaction& tx : pblock->vtx) { SyncWithWallets(tx, pblock); }
...
bool DisconnectBlocksAndReprocess(int blocks) {
  ...
  for (int i = 0; i <= blocks; i++) DisconnectTip(state);
...
bool DisconnectBlockAndInputs(CValidationState& state, CTransaction txLock) {
  ...
  for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0 && !foundConflictingTx && i < 6; i++) {
    ...
    CBlock block;
    if (!ReadBlockFromDisk(block, BlockReading)) return state.Abort(_("Failed to read block"));
...
static bool ActivateBestChainStep(CValidationState& state, CBlockIndex* pindexMostWork, CBlock* pblock,
  ...
  while (chainActive.Tip() && chainActive.Tip() != pindexFork) {
    if (!DisconnectTip(state)) return false;
  }
...
bool ActivateBestChain(CValidationState& state, CBlock* pblock, bool fAlreadyChecked) {
  ...
  do {
    ...
    while (true) {
      ...
      if (pindexMostWork == nullptr || pindexMostWork == chainActive.Tip()) return true;
...
CBlockIndex* AddToBlockIndex(const CBlock& block) {
  ...
  if (miPrev != mapBlockIndex.end()) {
    ...
    if (pindexNew->IsProofOfStake()) {
      if (!mapProofOfStake.count(hash)) LogPrintf("AddToBlockIndex() : hashProofOfStake not found in map \n");
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
      LogPrintf("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=%lld\n",
                pindexNew->nHeight, nStakeModifier);
  }
  pindexNew->nChainWork = (pindexNew->pprev ? pindexNew->pprev->nChainWork : 0) + GetBlockProof(*pindexNew);
  pindexNew->RaiseValidity(BLOCK_VALID_TREE);
  if (pindexBestHeader == nullptr || pindexBestHeader->nChainWork < pindexNew->nChainWork) pindexBestHeader = pindexNew;

  // update previous block pointer
  if (pindexNew->nHeight) pindexNew->pprev->pnext = pindexNew;
...
bool ReceivedBlockTransactions(const CBlock& block, CValidationState& state, CBlockIndex* pindexNew,
  ...
  if (block.IsProofOfStake()) pindexNew->SetProofOfStake();
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev) {
  ...
  if (!Checkpoints::CheckBlock(nHeight, hash))
    return state.DoS(100, error("%s : rejected by checkpoint lock-in at %d", __func__, nHeight), REJECT_CHECKPOINT,
                     "checkpoint mismatch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp,
  ...
  if (block.GetHash() != Params().HashGenesisBlock()) {
    ...
    if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
      ...
      return state.DoS(100,
                       error("%s : prev block %s is invalid, unable to add block %s", __func__,
  ...
  if (block.IsProofOfStake()) {
    ...
    // XXX Re-enable later
    // if (!stake)            return error("%s: null stake ptr", __func__);
  ...
  if (pindex->nStatus & BLOCK_HAVE_DATA) {
    ...
    // return state.DoS(20, error("AcceptBlock() : already have block %d %s", pindex->nHeight,
    // pindex->GetBlockHash().ToString()), REJECT_DUPLICATE, "duplicate");
    return true;
  }
  ...
  try {
    ...
    if (dbp == nullptr)
      if (!WriteBlockToDisk(block, blockPos)) return state.Abort("Failed to write block");
    if (!ReceivedBlockTransactions(block, state, pindex, blockPos))
      return error("AcceptBlock() : ReceivedBlockTransactions failed");
  } catch (std::runtime_error& e) { return state.Abort(std::string("System error: ") + e.what()); }
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp) {
...

  {
    ...
    if (!checked) { return error("%s : CheckBlock FAILED for block %s", __func__, pblock->GetHash().GetHex()); }

    // Store to disk
    CBlockIndex* pindex = nullptr;
    bool ret = AcceptBlock(*pblock, state, &pindex, dbp, checked);
    if (pindex && pfrom) { mapBlockSource[pindex->GetBlockHash()] = pfrom->GetId(); }
    CheckBlockIndex();
    if (!ret) return error("%s : AcceptBlock FAILED", __func__);
  }

  if (!ActivateBestChain(state, pblock, checked)) return error("%s : ActivateBestChain failed", __func__);

  if (pwalletMain) {
    // If turned on MultiSend will send a transaction (or more) on the after maturity of a stake
    if (pwalletMain->isMultiSendEnabled()) pwalletMain->MultiSend();

    // If turned on Auto Combine will scan wallet for dust to combine
    if (pwalletMain->fCombineDust) pwalletMain->AutoCombineDust();
  }
  /*
  LogPrintf("%s : ACCEPTED Block %ld in %ld milliseconds with size=%d\n", __func__, GetHeight(), GetTimeMillis() -
  nStartTime, pblock->GetSerializeSize(SER_DISK, CLIENT_VERSION));
*/
  return true;
}
...
bool TestBlockValidity(CValidationState& state, const CBlock& block, CBlockIndex* const pindexPrev, bool fCheckPOW,
  ...
  if (!ContextualCheckBlockHeader(block, state, pindexPrev)) return false;
  if (!CheckBlock(block, state, fCheckPOW, fCheckMerkleRoot)) return false;
  if (!ContextualCheckBlock(block, state, pindexPrev)) return false;
  if (!ConnectBlock(block, state, &indexDummy, viewNew, true)) return false;
...
bool InitBlockIndex() {
  ...
  if (!fReindex) {
    try {
      ...
      if (!WriteBlockToDisk(block, blockPos)) return error("LoadBlockIndex() : writing genesis block to disk failed");
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos* dbp) {
  ...
  try {
    ...
    while (!blkdat.eof()) {
      ...
      try {
        ...
        // read size
        blkdat >> nSize;
        if (nSize < 80 || nSize > MAX_BLOCK_SIZE_CURRENT) continue;
      ...
      try {
        ...
        if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
          CValidationState state;
          if (ProcessNewBlock(state, nullptr, &block, dbp)) nLoaded++;
  ...
  } catch (std::runtime_error& e) { AbortNode(std::string("System error: ") + e.what()); }
...
void static CheckBlockIndex() {
  ...
  while (pindex != nullptr) {
    ...
    // HAVE_DATA is equivalent to VALID_TRANSACTIONS and equivalent to nTx > 0 (we stored the number of transactions in
    // the block)
    assert(!(pindex->nStatus & BLOCK_HAVE_DATA) == (pindex->nTx == 0));
    assert(((pindex->nStatus & BLOCK_VALID_MASK) >= BLOCK_VALID_TRANSACTIONS) == (pindex->nTx > 0));
    if (pindex->nChainTx == 0)
      assert(pindex->nSequenceId == 0);  // nSequenceId can't be set for blocks that aren't linked
    // All parents having data is equivalent to all parents being VALID_TRANSACTIONS, which is equivalent to nChainTx
    // being set.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived) {
  ...
  if (strCommand == "version") {
    ...
    if (pfrom->fInbound && addrMe.IsRoutable()) { SeenLocal(addrMe); }
  ...
  // getaddr message mitigates the attack.
    ...
    for (uint256& hash : vtxid) {
      ...
      if ((pfrom->pfilter && pfrom->pfilter->IsRelevantAndUpdate(tx)) || (!pfrom->pfilter)) vInv.push_back(inv);
      if (vInv.size() == MAX_INV_SZ) {
        pfrom->PushMessage("inv", vInv);
        vInv.clear();
      }
    ...
    if (bPingFinished) { pfrom->nPingNonceSent = 0; }
           ...
           (strCommand == "filterload" || strCommand == "filteradd" || strCommand == "filterclear")) {
  ...
  while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
    ...
    try {
      ...
      if (strstr(e.what(), "end of data")) {
        ...
        LogPrintf(
            "ProcessMessages(%s, %u bytes): Exception '%s' caught, normally caused by a message being shorter than its "
            "stated length\n",
...
bool SendMessages(CNode* pto, bool fSendTrickle) {
  {
    ...
    if (!pto->fDisconnect && state.vBlocksInFlight.size() > 0 &&
        state.vBlocksInFlight.front().nTime <
            nNow - 500000 * Params().TargetSpacing() * (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
