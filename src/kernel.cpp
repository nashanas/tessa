...
static bool SelectBlockFromCandidates(vector<pair<int64_t, uint256> >& vSortedByTimestamp,
  ...
  *pindexSelected = (const CBlockIndex*)0;
  for (const auto& item : vSortedByTimestamp) {
    ...
    if (fSelected && pindex->GetBlockTime() > nSelectionIntervalStop) break;
    ...
    if (pindex->IsProofOfStake()) hashSelection >>= 32;
  ...
  for (int nRound = 0; nRound < min(64, (int)vSortedByTimestamp.size()); nRound++) {
    ...
    if (GetBoolArg("-printstakemodifier", false))
      LogPrintf("ComputeNextStakeModifier: selected round %d stop=%s height=%d bit=%d\n", nRound,
                DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nSelectionIntervalStop).c_str(), pindex->nHeight,
...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight,
  ...
  nStakeModifier = 0;
  if (!mapBlockIndex.count(hashBlockFrom)) return error("GetKernelStakeModifier() : block not indexed");
...
bool GetCoinAge(const CTransaction& tx, const unsigned int nTxTime, uint64_t& nCoinAge) {
  ...
  for (const CTxIn& txin : tx.vin) {
    ...
    if (prevblock.nTime + Params().StakeMinAge() > nTxTime) continue;  // only count coins meeting min age requirement
