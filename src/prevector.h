 ...
 */
template <unsigned int N, typename T, typename Size = uint32_t, typename Diff = int32_t> class prevector {
 public:
 ...
 public:
  void assign(size_type n, const T &val) {
    clear();
    if (capacity() < n) { change_capacity(n); }
    while (size() < n) {
      _size++;
  ...
  template <typename InputIterator> prevector(InputIterator first, InputIterator last) : _size(0) {
    size_type n = last - first;
    change_capacity(n);
    while (first != last) {
      _size++;
  ...
  void resize(size_type new_size) {
    if (size() > new_size) { erase(item_ptr(new_size), end()); }
    if (new_size > capacity()) { change_capacity(new_size); }
    while (size() < new_size) {
      _size++;
  ...
  void reserve(size_type new_capacity) {
    if (new_capacity > capacity()) { change_capacity(new_capacity); }
  }
  ...
  iterator insert(iterator pos, const T &value) {
    ...
    if (capacity() < new_size) { change_capacity(new_size + (new_size >> 1)); }
  ...
  void insert(iterator pos, size_type count, const T &value) {
    ...
    _size += count;
    for (size_type i = 0; i < count; i++) { new (static_cast<void *>(item_ptr(p + i))) T(value); }
  }
